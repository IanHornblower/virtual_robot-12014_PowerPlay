...
public class FollowPurePursuitTrajectory extends Action {
    ...
    SqrtControl xController = new SqrtControl(new SqrtCoefficients(0.135, 0.0, 0.0));
    SqrtControl yController = new SqrtControl(new SqrtCoefficients(0.135, 0.0, 0.1));

    AngleController hController = new AngleController(new BasicPID(new PIDCoefficients(2, 0, 0)));
    ...
    public void runAction() throws InterruptedException {
        ...
        double forward = Range.clip(xController.calculate(distance, 0), -0.5, 0.5);
        ...
        isComplete =
                        Math.abs(path.get(path.size()-2).x - robotPose.x) < 3 &&
                        Math.abs(path.get(path.size()-2).y - robotPose.y) < 3 &&
